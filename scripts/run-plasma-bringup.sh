...
"$PYTHON" scripts/finalize-plasma-dbus.py "$BASH_COMPAT_C"
...
"$PYTHON" scripts/finalize-plasma-x11-poll.py "$BASH_COMPAT_C"
"$PYTHON" scripts/finalize-plasma-kde-fs.py "$BASH_COMPAT_C"
"$PYTHON" scripts/finalize-plasma-threads.py "$BASH_COMPAT_C"
...
if [ "$ROOTFS_KIND" = "plasma-x11" ]; then
    ...
    echo "Default boot mode: automatic background Xorg + Plasma session"
    echo "Shell override: add 'nox.shell=1' to the Limine cmdline (or select the shell entry)"
    echo "Expected framebuffer proof when Xorg maps video memory:"
    echo "  [linux:fbdev] synthetic sysfs subsystem -> bus/platform"
    echo "  [linux:fbdev] mapped /dev/fb0 into userspace"
    echo "If graphical startup exits, Bash remains available as a nox# debug shell."
